...
[i.d.(V), dim V] where i.d.(V) is the isotypical decomposition of the space V and dim V its dimension. 
...
$M_{k,j}(\Gamma[2])$, $S_{k,j}(\Gamma[2])$, $E_{k,j}(\Gamma[2])$, $E^{(F)}_{k,j}(\Gamma[2])$, $E^{(Q)}_{k,j}(\Gamma[2])$, 
$S^{(P)}_{k,j}(\Gamma[2])$, 
